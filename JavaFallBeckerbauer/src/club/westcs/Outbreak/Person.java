...
public class Person {
	 ...
	 * 		this.attribute does not work woth static attributes
	...
	//Methods
		...
		public Person baby(Person other) {
			if(other.isGender() != this.isGender() & rand.nextInt(3) == 1) { //sets the birth rate
				return new Person(this.infected || other.isInfected()); 	//return a new baby that will be infected if either parent is infected
			}
			return null;
		}
